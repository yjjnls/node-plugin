...
class Plugin  {
	...
	constructor(name, dir ,notify) {
		if(!node_plugin){
			...
			this.__addon__ = path.normalize( path.resolve(__dirname,module) )
			if (!fs.existsSync(this.__addon__)){
				throw Error(`${this.__addon__} not exists.`)
			}
...
Plugin.__file__=`${__dirname}/../bin/${Arch.current}/plugin.node`
